...
namespace App.View.Task
{
    public partial class frmOutStock : BaseForm
    {
        ...
        {
            DataTable dt = bll.FillDataTable("WCS.SelectTask", new DataParameter[] { new DataParameter("{0}", string.Format("WCS_TASK.State in('0','100') and WCS_TASK.TaskType='12'")) });
            bsMain.DataSource = dt;
        }
        private void BindData(string filter)
        {
            DataTable dt = bll.FillDataTable("WCS.SelectTask", new DataParameter[] { new DataParameter("{0}", string.Format("WCS_TASK.TaskType='12' and {0}", filter)) });
            bsMain.DataSource = dt;
        }
        ...
        private void toolStripButton_Stop_Click(object sender, EventArgs e)
        {
            ...
            if (TaskNo != "")
            {
                ...
                MCP.Logger.Info("暂停出库单：" + ((DataRowView)dgvMain.Rows[0].DataBoundItem).Row["BillID"].ToString() + "任务！");
                this.BindData();
            }
        }
        ...
        private void toolStripButton_Start_Click(object sender, EventArgs e)
        {
            ...
            if (TaskNo != "")
            {
                ...
                MCP.Logger.Info("开始出库单：" + ((DataRowView)dgvMain.Rows[0].DataBoundItem).Row["BillID"].ToString() + "任务！");
                this.BindData();
            }
        }
